...
def _user(role: str = "consultant") -> User:
    return User(id=new_id(), email=f"{role}@x.com",
...
def test_export_includes_top_level_sections():
    ...
    project = create_project(uow, consultant, ProjectCreate(
        name="P", domain="legal", description="desc",
    ))
    bundle = export_project(uow, consultant, project.id)
    assert bundle["schema_version"] == "1.0"
    assert {"exported_at", "exported_by", "project", "documents",
            "extractions", "comments", "coverage"} <= set(bundle)
...
def test_export_includes_all_child_entities():
    ...
    uow.documents.add(Document(
        project_id=project.id, original_name="d.docx", relative_path="docs/x.docx",
    ...
    uow.extractions.add(Extraction(
        ...
        llm_model="mock", content={"_meta": {}, "nome_projeto": "X"},
        created_by=None,
    ))
...
def test_export_rejects_invisible_project():
    ...
    cons_b = User(id=new_id(), email="b@x.com",
                  password_hash="x", name="B", role="consultant")
